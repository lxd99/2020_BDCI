...
def evaluate(Y_true, Y_preds):
    ...
    for i in range(MODEL_N // 2):
        cpu_true, job_true = Y_true[:, i * 2] / 10, Y_true[:, i * 2 + 1]  # shape: (n,)
        cpu_preds, job_preds = Y_preds[:, i * 2] / 10, Y_preds[:, i * 2 + 1]  # shape: (n,)
...
train = train_df.copy()
test = test_df.copy()
my_feature = ['CPU_USAGE', 'MEM_USAGE', 'DISK_USAGE']
for i in my_feature:
    train_df[i] = train_df[i] * 10
    test_df[i] = test_df[i] * 10
...
fjobs = []
# 差分
my_feature2 = ['CPU_USAGE', 'MEM_USAGE', 'DISK_USAGE']
for i in range(1, 5):
...
for train_idx, validate_idx in kf:
    ...
    for i in range(MODEL_N):
        ...
        # 验证集
        valid_pred = bst.predict(X_valid, num_iteration=bst.best_iteration_)
        valid_pred[valid_pred < 0] = 0
        # valid_pred[valid_pred > 200] = 200
        if i % 2 == 0:
            valid_pred[valid_pred > 1000] = 1000
        valid_pred = valid_pred.astype(np.int)
        oof[validate_idx, i] = valid_pred

        # 测试集
        test_pred = bst.predict(test_df, num_iteration=bst.best_iteration_)
        test_pred[test_pred < 0] = 0
        if i % 2 == 0:
            test_pred[test_pred > 1000] = 1000
...
pbar.close()
for i in range(1, 6):
    sample_df['CPU_USAGE_' + str(i)] = sample_df['CPU_USAGE_' + str(i)] / 10
# 转为整型
